...
typedef enum { RED=0, ORANGE, YELLOW, GREEN, BLUE, MULTI } Color;
const int NUMCOLORS = 6;

/* Bots may assume that these correspond to their integer values. */
typedef enum { ONE=1, TWO, THREE, FOUR, FIVE } Value;

// Game modes that bots may support
typedef enum { NORMAL, TRICKY, DIFFICULT, VERYDIFFICULT } GameMode;
...
struct Server {
    ...
    int runGame(const BotFactory &botFactory, int numPlayers, Hanabi::GameMode mode);
     ...
     * the game. */
    int runGame(const BotFactory &botFactory, int numPlayers, Hanabi::GameMode mode, const std::vector<Card>& stackedDeck);
...
struct Bot {
    ...
    virtual bool botCanPlay(GameMode mode) = 0;
};

struct BotFactory {
    virtual Bot *create(int index, int numPlayers, int handSize, Hanabi::GameMode mode) const = 0;
