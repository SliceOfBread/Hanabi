...
struct Statistics {
    int games;
    int totalScore;
    int scoreDistribution[(5*Hanabi::NUMCOLORS)+1];
    int mulligansUsed[4];
};

static void run_iterations(int numberOfPlayers, Hanabi::GameMode mode, int iterations, Statistics &global_stats)
{
	...
	if (debugMode) {
        ...
        try {
            score = server.runGame(botFactory, numberOfPlayers, mode);
        } catch (const std::runtime_error& e) {
            std::clog << "Caught fatal exception \"" << e.what() << "\" from Hanabi server" << std::endl;
    ...
    #pragma omp critical (update_and_dump_stats)
    {
        ...
        for (int s=0; s <= (5*Hanabi::NUMCOLORS); ++s) {
...
static void dump_stats(Statistics stats, bool produceHistogram, Hanabi::GameMode mode)
{
    #pragma omp critical (update_and_dump_stats)
    {
        const double dgames = stats.games;
        const int perfectGames = stats.scoreDistribution[(mode==Hanabi::NORMAL) ? 25 : 30];
...
static void usage(const char *why)
{
    std::cout << why << "\n"
              ...
              << "        --players <2..10>\n"
              << "        --mode <normal|tricky|diff|verydiff>\n"
              << "        --games <1000..2000000000>\n"
              << "        --every <1000..2000000000>\n";
    std::exit(EXIT_FAILURE);
}

static void run_one_stacked_deck_game(int numberOfPlayers, Hanabi::GameMode mode)
{
    ...
    int score = server.runGame(botFactory, numberOfPlayers, mode, stackedDeck);
...
int main(int argc, char **argv)
{
    ...
    Hanabi::GameMode mode = Hanabi::NORMAL;
    ...
    for (int i=1; i < argc; ++i) {
        ...
        } else if (argv[i] == std::string("--mode")) {
            if (i+1 >= argc) usage("Option --mode requires an argument.");
	    ++i;
	    if (argv[i] == std::string("normal")) {
		mode = Hanabi::NORMAL;
	    } else if (argv[i] == std::string("tricky")) {
		mode = Hanabi::TRICKY;
	    } else if (argv[i] == std::string("diff")) {
		mode = Hanabi::DIFFICULT;
	    } else if (argv[i] == std::string("verydiff")) {
		mode = Hanabi::VERYDIFFICULT;
	    } else {
		// not a known mode
		usage ("Invalid mode.");
	    }
    ...
    if (stackTheDeck) {
        run_one_stacked_deck_game(numberOfPlayers, mode);
        return 0;
    }
    ...
    if (!quiet) {
        ...
        BotFactory<BOTNAME> botFactory;
        server.setLog(&std::cerr);
        int score = server.runGame(botFactory, numberOfPlayers, mode);
    ...
    for (int i=0; i < numberOfGames / every; ++i) {
        run_iterations(numberOfPlayers, mode, every, stats);
        assert(stats.games % every == 0);
        dump_stats(stats, produceHistogram, mode);
    }

    if ((numberOfGames % every) != 0) {
        run_iterations(numberOfPlayers, mode, (numberOfGames % every), stats);
        dump_stats(stats, produceHistogram, mode);
    }
